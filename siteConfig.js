...
const schema = {
  ...
  allowedOrigins: {
    doc:
      "When allowedOrigins is defined, only requests sent from one of the domains listed will be accepted.",
  ...
  akismet: {
    ...
    authorEmail: {
      doc:
        "Name of the field to be used as the entry's author's email in Akistmet",
      format: String,
      default: "",
    },
    authorUrl: {
      doc:
        "Name of the field to be used as the entry's author's URL in Akistmet",
      format: String,
      default: "",
    },
    content: {
  ...
  githubAuth: {
    ...
    required: {
      doc:
        "Whether GitHub Auth is required for an entry to be accepted. This is only included for backwards compatibility with the v2 API. For the v3 API, please use the `auth.required` option instead.",
      format: Boolean,
      default: false,
    },
  },
  ...
  notifications: {
    enabled: {
      doc:
        "Whether email notifications are enabled. This allows users to subscribe to future comments on a thread. A [Mailgun](http://mailgun.com) account is required.",
      format: Boolean,
      default: false,
    },
    apiKey: {
  ...
  reCaptcha: {
    ...
    version: {
      doc:
        "Define what reCaptcha Version your key is for v2 or v3",
      format: String,
      default: "v2",
    },
    siteKey: {
