...
const reCaptcha3 = require("express-recaptcha").RecaptchaV3;
    ...
    staticman
      ...
      .then((siteConfig) => {
        ...
        const reCaptchaOptions = {
          siteKey: siteConfig.get("reCaptcha.siteKey"),
        ...
        if (
          ...
          !reCaptchaOptions.secret
        ) {
          return reject(errorHandler("RECAPTCHA_MISSING_CREDENTIALS"));
        }

        const recaptcha =
          siteConfig.get("reCaptcha.version") === "v3"
            ? new reCaptcha3(
                reCaptchaOptions.siteKey,
                reCaptchaOptions.secret,
                {
            ...
            : new reCaptcha(reCaptchaOptions.siteKey, reCaptchaOptions.secret, {
                callback: "cb",
              });
...
function sendResponse(res, data) {
  ...
  if (error && error._smErrorCode) {
    const errorCode = errorHandler
      .getInstance()
      .getErrorCode(error._smErrorCode);
    const errorMessage = errorHandler
      .getInstance()
      .getMessage(error._smErrorCode);
