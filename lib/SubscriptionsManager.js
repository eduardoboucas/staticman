...
SubscriptionsManager.prototype._getMember = function (entryId) {
  const listAddress = this._getListAddress(entryId)

  return new Promise((resolve, reject) => {
    this.mailAgent.lists(listAddress).members().list(function (err, members) {
      if (err && (err.statusCode !== 404)) {
        return reject(err)
      }

      if (err || !members || !members.items || !members.items.length) {
        return resolve(null)
      }

      return resolve(members.items[0].address)
    })
  })
}

SubscriptionsManager.prototype.delete = function (entryId) {
  const listAddress = this._getListAddress(entryId)

  return new Promise((resolve, reject) => {
    this.mailAgent.lists(listAddress).delete((err) => {
      if (err && (err.statusCode !== 404)) {
        return reject(err)
      }
...
SubscriptionsManager.prototype.moveMember = function (fromEntryId, toEntryId) {
  return this._getMember(fromEntryId).then(address => {
    if (address) {
      return this.set(toEntryId, address)
    }
  })
}

SubscriptionsManager.prototype.send = function (entryId, fields, options, siteConfig, template) {
