...
GitHub.prototype.writeFile = function (filePath, data, branch, commitTitle) {
  return this._writeFile(filePath, data, branch, commitTitle, this.options.username)
}


GitHub.prototype._writeFile = function (filePath, data, branch, commitTitle, username) {
  branch = branch || this.options.branch
...
/* Bug: Repo name could be this.options.repository + '-' + num for some positive int num */
GitHub.prototype.forkAndWriteFileAndSendPR = function (filePath, data, branch, commitTitle, commitBody, botUsername) {
  commitTitle = commitTitle || 'Add Staticman file'
  commitBody = commitBody || ''
  ...
  return this.api.repos.fork({
    ...
    return this._writeFile(filePath, data, branch, commitTitle, botUsername)
  }).then(res => {
    return this.api.pullRequests.create({
      user: this.options.username,
      repo: this.options.repository,
      title: commitTitle,
      head: botUsername + ':' + branch,
      base: this.options.branch,
      body: commitBody
    })
  }).catch(err => {
