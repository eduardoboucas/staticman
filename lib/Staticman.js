...
Staticman.prototype._getConfig = function (force) {
  ...
  if (!this.configFile) return Promise.reject('NO_CONFIG_PATH')

  return this.github.readFile(this.configFile.file).then(data => {
    const config = objectPath.get(data, this.configFile.path)
...
Staticman.prototype._processSubscriptions = function (fields, options) {
  ...
  return subscriptions.get(options.parent).then(list => {
    if (list) {
      const notifications = new Notification(mailgun)

      notifications.send(list, fields, options, {
...
Staticman.prototype.processEntry = function (fields, options) {
  // Keep a copy of the original fields and options
  ...
  return this._getConfig().then(res => {
    ...
    if (this.siteConfig.get('moderation')) {
      ...
      // If moderation isn't enabled, we can process subscriptions right away.
      this._processSubscriptions(this.fields, this.options)
    }
