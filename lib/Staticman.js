...
class Staticman {
  ...
  processMerge (fields, options) {
    /*
     * Requests to process a merge will only come via a webhook, not a user. As such, no need to
     * use the ErrorHandler when returning errors.
     */
    return this.getSiteConfig().then(config => {
      const subscriptions = this._initialiseSubscriptions()

      return subscriptions.send(options.parent, fields, options, this.siteConfig).catch(err => {
