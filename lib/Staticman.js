...
class Staticman {
  ...
  _generateReviewBody (fields) {
    ...
    if (this.options["origin"]) {
      table.push(['page', this.options["origin"]])
    }
  ...
  processEntry (fields, options) {
    ...
    return this.getSiteConfig().then(config => {
      ...
      if (this.siteConfig.get('moderation')) {
        ...
        // Subscribe user to "accepted" notification, if applicable
        if (subscriptions && options.parent && options.subscribe && this.fields[options.subscribe]) {
          subscriptions.set(newBranch, this.fields[options.subscribe]).catch(err => {
        ...
        subscriptions.send(options.parent, fields, options, this.siteConfig, 'reply')
        
        // Subscribe user to reply notifications, if applicable
        if (subscriptions && options.parent && options.subscribe && this.fields[options.subscribe]) {
  ...
  processMerge (fields, options, branchName) {
    this.fields = Object.assign({}, fields)
    this.options = Object.assign({}, options)
    this.branchName = Object.assign("", branchName)
    ...
    return this.getSiteConfig().then(config => {
      ...
      let queue = [
        subscriptions.send(options.parent, fields, options, this.siteConfig, 'reply'),
        subscriptions.send(branchName, fields, options, this.siteConfig, 'accepted')
      ]
      return Promise.all(queue).then(() => {
        return subscriptions.moveMember(branchName, options.parent)
      })
    }).catch(err => {
  ...
  processClose (fields, options, branchName) {
    this.fields = Object.assign({}, fields)
    this.options = Object.assign({}, options)
    this.branchName = Object.assign("", branchName)
    ...
    return this.getSiteConfig().then(config => {
      ...
      return subscriptions.delete(branchName)
    }).catch(err => {
      return Promise.reject(errorHandler('ERROR_PROCESSING_CLOSE', {
        err,
        instance: this
      }))
    })
