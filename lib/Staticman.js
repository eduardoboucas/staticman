...
Staticman.prototype.processEntry = function (fields, options) {
  ...
  return this.getSiteConfig().then(config => {
    ...
    // Check origin
    const allowedOrigins = this.siteConfig.get('allowedOrigins')
    ...
    if (allowedOrigins) {
      if (this.options.origin) {
        ...
        if (!validOrigin) {
          return Promise.reject(errorHandler('INVALID_ORIGIN'))
        }
      } else {
        return Promise.reject(errorHandler('MISSING_ORIGIN'))
      }
    }
