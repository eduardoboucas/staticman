...
Staticman.prototype._applyTransforms = function (fields) {
  // Keep a copy of the original fields for future usage
  this._fieldsOriginal = Object.assign({}, fields)
...
Staticman.prototype._processSubscriptions = function (fields, options) {
  ...
  return subscriptions.get(options.parent).then(list => {
    if (list) {
      ...
      notifications.send(list, fields, options, this.siteConfig)
    }
...
Staticman.prototype.process = function (fields, options) {
  return this._getConfig().then(res => {
    ...
    this._processSubscriptions(this._fieldsOriginal, options)
