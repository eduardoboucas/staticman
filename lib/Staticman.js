...
Staticman.prototype.processEntry = function (fields, options) {
  ...
  return this.getSiteConfig().then(config => {
    ...
    if (this.siteConfig.get('moderation')) {
      ...
      if (this.siteConfig.get('fork')) {
        const botUsername = config.get('botUsername')
        return this.github.forkAndWriteFileAndSendPR(filePath, data, newBranch, commitMessage, this._generatePRBody(fields), botUsername)
      }
