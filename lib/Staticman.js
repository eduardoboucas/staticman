...
Staticman.prototype._checkAuth = function () {
  ...
  if (this.parameters.version === '2' && !this.options['github-token']) {
    return Promise.reject(errorHandler('GITHUB_AUTH_TOKEN_MISSING'))
  }

  if (this.parameters.version !== '2' && !this.options['auth-token']) {
    return Promise.reject(errorHandler('AUTH_TOKEN_MISSING'))
  }

  const oauthToken = this.parameters.version === '2'
    ? RSA.decrypt(this.options['github-token'])
    : RSA.decrypt(this.options['auth-token'])

  if (!oauthToken) {
    return this.parameters.version === '2'
      ? Promise.reject(errorHandler('GITHUB_AUTH_TOKEN_INVALID'))
      : Promise.reject(errorHandler('AUTH_TOKEN_INVALID'))
  }

  const git = gitFactory.create(this.parameters.service, {oauthToken})
